...
\end{align}
where the first two boundary conditions come from the virtual RCM constraint, while the third results from the assumption that the bending moment at the needle tip is negligible. \added[id=rt]{Using higher-order polynomials would introduce additional degrees of freedom, and requiring more conditions to uniquely determine the coefficients. Solution to the above system of equations therefore depends on the choice of polynomial order, which is not ideal. Later, in~\cref{chap:chap-3}, a finite element solution will be developed to solve the proposed needle-tissue interaction model.}
...
\subsubsection{Force data collection and image analysis} % (fold)
\label{sec:chap-2-force-data-img-analysis}
\begin{figure}[h]
  \centering
  \includegraphics[width = \columnwidth]{manuscripts/IROS_2021/figures/experiment_setup.png}
  \caption{Experiment setup to study the needle-tissue interaction during virtual RCM motion. A testbed is built to hold the gel phantom and attach the force sensor. A DSLR camera is used to capture videos of the needle inside of the gel.}
  \label{fig:chap-2-experiment-setup}
\end{figure}
\added[id=ii]{
Experiments are carried out to validate the theoretical model of needle-tissue interaction, as well as to gather force information. The experiment setup is shown in~\cref{fig:chap-2-experiment-setup}. To avoid excessive modification to the current robot, a 6-axis F/T sensor (Nano25, ATI Industrial Automation, USA) is attached at the bottom of a testbed, which can hold different types of gel phantoms. The gel phantoms are made from liquid plastic with softeners and hardeners as modifiers (M-F Manufacturing Company, USA). Force collection is performed on a separate PC using a DAQ card. Noisy force data are smoothened with a moving average filter to increase the signal-to-noise ratio to a desired level, and the net forces are compared across different scenarios.}

\added[id=rt]{A calibrated DSLR camera (Nikon DSC D5200, Japan) is positioned parallel to the sagittal plane for capturing videos of the needle and tissue response during the virtual RCM motion. To find the instant center of motion (COM), each video file is processed frame-by-frame by converting all frames to black and white images with some desired threshold level. The images are then overlaid with each other to find the common pixels within a region of interest (ROI), and the group of white pixels that are present in all frames during the entire movie defines the approximate COM region of the needle. The center pixel of each COM region is taken as the actual COM point.}
...
Note that any two of the following quantities: virtual RCM location vRCM, bottom stage displacement $m_2$, and the needle incident angle $\theta$, determine the third, so any two variables could have been chosen as independent. Here, \replaced[id=ak]{it was}{we} decided that angulation is a more intuitive choice for the clinician during the operation, along with the virtual RCM position -- \replaced[id=ak]{the}{our} main variable of interest, so they are chosen as the independent variables. Hence, a deeper RCM placement with the same angulation will necessarily result in a greater linear displacement of the bottom stage.
